...
Praktikum P01 memberikan pemahaman mengenai penggunaan Docker dalam lingkungan basis data, khususnya dalam menjalankan PostgreSQL, MongoDB, dan Redis menggunakan Docker Compose.

Praktikum juga menunjukkan pentingnya penggunaan volume untuk menjaga persistensi data, port mapping untuk menghubungkan container dengan host, healthcheck untuk memastikan kesiapan service, serta pengelolaan credential agar lebih aman ketika project disimpan dalam repository Git.

Selain itu, penggunaan `psql` dan DBeaver memiliki keunggulan masing-masing. `psql` efektif untuk operasi berbasis terminal dan scripting, sedangkan DBeaver lebih sesuai untuk eksplorasi database secara visual.

Hasil tantangan tambahan juga menunjukkan bahwa penggunaan index dapat memberikan peningkatan performa yang signifikan pada proses pencarian data, terutama ketika database memiliki jumlah baris yang besar.
=======
# DOCKER
